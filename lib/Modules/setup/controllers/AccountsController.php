		...
		try {
			...
			foreach($tmpAccts as $key => $acctName) {
				if (Account_Util::exists($acctName)) {
					...
					$acctId = Account_Util::create($acctName);
					...
					if ($account->getUsername() == 'api') {
						$log->debug('Special account "api" created. Adding credentials to local configuration file');
						...
						$configArray['ws']['api']['savory']['api']['username'] = $account->getUsername();
						$configArray['ws']['api']['savory']['api']['password'] = $account->password;
