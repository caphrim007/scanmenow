	...
	protected function createQueue($name) {
		...
		try {
			if (file_exists($configPath) && !is_writable($configPath)) {
				throw new Exception('The config file exists, but is not writable by savory');
			} else if (!file_exists($configPath) && !is_writable(dirname($configPath))) {
				throw new Exception('The local config file does not exist and the local config directory is not writable, so savory cannot create it');
			}
